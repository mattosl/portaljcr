...
public class ChamadoDataModel extends LazyDataModel<Chamado> {

	private static Logger log = Logger.getLogger(ChamadoDataModel.class);
	...
	public List<Chamado> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
		try {
			this.setRowCount(chamadoBusiness.obterQtdChamado(getFiltro()));
			if(getRowCount() > 0){
				this.setDataSource(chamadoBusiness.listarChamadoPaginado(first, pageSize, getFiltro()));
			}
